...
public:
    ...
    bool isRunning();
    void run();
	shared_ptr<Time> addTicker(shared_ptr<Session> session, int h, int m, int s, int ms);
	shared_ptr<Time> addTimer(shared_ptr<Session> session, int h, int m, int s, int ms);
	void deleteSession(shared_ptr<Session> session);
...
private:
	struct EpollEventArg {
		...
		EpollEventArg(shared_ptr<Time> t, shared_ptr<Session> session) : t(t), session(session) {};
	};
private:
    ...
    int waitTime;
    int epollEventNum;
    unordered_map<int, shared_ptr<Session>> sessionManager;
